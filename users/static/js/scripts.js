        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#profileAlertContainer').html('<div class="alert alert-danger">' + response.message + '</div>');
                    $.each(response.errors, function(field, errors) {
                        $('#error-' + field).html(errors.join('<br>'));
                    });
                }
            ...
            error: function(xhr, status, error) {
                $('#profileAlertContainer').html('<div class="alert alert-danger">An error occurred. Please try again.</div>');
            }
        });
    });
});